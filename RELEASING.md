...
The stable `Farmer's Delight compatibility` check covers the newest compatible stable Farmer's Delight Refabricated releases discovered from Modrinth for this branch, up to five versions. Matrix launches use the newest stable Fabric API for the branch's Minecraft version, while release compilation remains pinned to `fabric_version`. Require that aggregate check rather than the individual versioned matrix jobs.
