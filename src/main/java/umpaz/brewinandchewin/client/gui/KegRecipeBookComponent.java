...
public class KegRecipeBookComponent extends RecipeBookComponent<KegMenu> {
    ...
    protected void selectMatchingRecipes(RecipeCollection possibleRecipes, StackedItemContents stackedItemContents) {
        possibleRecipes.selectRecipes(stackedItemContents, recipeDisplay -> findRecipe(recipeDisplay) != null);
    ...
    protected boolean isCraftingSlot(Slot slot) {
        return slot.index >= 0 && slot.index <= KegBlockEntity.CONTAINER_SLOT;
    }
    ...
    protected void fillGhostRecipe(GhostSlots ghostSlots, RecipeDisplay recipeDisplay, ContextMap contextMap) {
        ...
        this.lastRecipeDisplay = recipeDisplay;
    ...
    private void populateGhostRecipe(GhostSlots ghostSlots, RecipeDisplay recipeDisplay, ContextMap contextMap) {
        ...
        List<Ingredient> displayedRecipeIngredients = ingredients.subList(0, displayedIngredients);
        IngredientAvailability availability = this.analyzeIngredientAvailability(displayedRecipeIngredients, scale, inputSlots);
        ...
        for (int i = 0; i < displayedIngredients; ++i) {
            int missingCount = availability.missingCounts()[i];
            if (missingCount <= 0) {
                continue;
            }

            Ingredient ingredient = displayedRecipeIngredients.get(i);
            this.markInsufficientMatchingSlots(ingredient, inputSlots);
    ...
    private IngredientAvailability analyzeIngredientAvailability(List<Ingredient> ingredients, int requiredCount, int inputSlots) {
        int source = 0;
        int firstSlot = 1;
        int firstIngredient = firstSlot + inputSlots;
        int sink = firstIngredient + ingredients.size();
        int nodeCount = sink + 1;
        int totalDemand = requiredCount * ingredients.size();
        int[][] capacity = new int[nodeCount][nodeCount];

        for (int slotIndex = 0; slotIndex < inputSlots; ++slotIndex) {
            ItemStack stack = this.kegMenu.getSlot(slotIndex).getItem();
            if (stack.isEmpty()) {
                continue;
            }
            int slotNode = firstSlot + slotIndex;
            capacity[source][slotNode] = Math.min(stack.getCount(), totalDemand);
            for (int ingredientIndex = 0; ingredientIndex < ingredients.size(); ++ingredientIndex) {
                if (ingredients.get(ingredientIndex).test(stack)) {
                    capacity[slotNode][firstIngredient + ingredientIndex] = totalDemand;
                }
            }
        }
        for (int ingredientIndex = 0; ingredientIndex < ingredients.size(); ++ingredientIndex) {
            capacity[firstIngredient + ingredientIndex][sink] = requiredCount;
        }
        ...
        while (true) {
            ...
            while (!queue.isEmpty() && parent[sink] < 0) {
                ...
                for (int next = 0; next < nodeCount; ++next) {
                    if (parent[next] < 0 && capacity[node][next] - flow[node][next] > 0) {
                        parent[next] = node;
                        queue.addLast(next);
                    }
                }
        ...
        int[] missingCounts = new int[ingredients.size()];
        for (int ingredientIndex = 0; ingredientIndex < ingredients.size(); ++ingredientIndex) {
            missingCounts[ingredientIndex] = requiredCount - flow[firstIngredient + ingredientIndex][sink];
        }
        return new IngredientAvailability(missingCounts);
    }

    private int findAvailableGhostSlot(int preferredIndex, int inputSlots, boolean[] usedGhostSlots) {
        if (preferredIndex < inputSlots
                && !usedGhostSlots[preferredIndex]
                && this.kegMenu.getSlot(preferredIndex).getItem().isEmpty()) {
    ...
    private void markInsufficientMatchingSlots(Ingredient ingredient, int inputSlots) {
        for (int i = 0; i < inputSlots; ++i) {
            Slot slot = this.kegMenu.getSlot(i);
    ...
    private static void setCountedGhost(GhostSlotsInvoker invoker, Slot slot, ContextMap contextMap, SlotDisplay display, int count) {
        List<ItemStack> countedDisplays = display.resolveForStacks(contextMap).stream()
                .filter(stack -> !stack.isEmpty())
                .map(stack -> stack.copyWithCount((int) Math.min(Integer.MAX_VALUE, (long) stack.getCount() * count)))
                .toList();
        if (!countedDisplays.isEmpty()) {
            invoker.brewinandchewin$setResult(slot, contextMap, new CountedSlotDisplay(countedDisplays));
        }
    }
    ...
    private void addRequiredFluidGhost(GhostSlotsInvoker invoker, ContextMap contextMap) {
        ...
        if (requiredFluid.ingredient().matches(tankFluid)
                && tankFluid.unit().convertToLoader(tankFluid.amount()) >= requiredAmount) {
            return;
        }
        ...
        if (!containerDisplays.isEmpty()) {
            ...
            invoker.brewinandchewin$setResult(containerSlot, contextMap, new CountedSlotDisplay(containerDisplays));
        }
    }
    ...
    private record IngredientAvailability(int[] missingCounts) {
    }

    private record CountedSlotDisplay(List<ItemStack> stacks) implements SlotDisplay {
        @Override
        public <T> Stream<T> resolve(ContextMap contextMap, DisplayContentsFactory<T> factory) {
            if (factory instanceof DisplayContentsFactory.ForStacks<?> forStacks) {
                @SuppressWarnings("unchecked")
                DisplayContentsFactory.ForStacks<T> stackFactory = (DisplayContentsFactory.ForStacks<T>) forStacks;
                return this.stacks.stream().map(ItemStack::copy).map(stackFactory::forStack);
            }
            return Stream.empty();
        }
        ...
        for (RecipeDisplayEntry entry : selectedCollection.getRecipes()) {
            if (entry.id().equals(selectedId)) {
                ...
                if (selectedRecipe != null) {
                    ...
                    this.lastRecipeDisplay = entry.display();
                }
                return;
            }
        }
    ...
    public void extractMissingSlotOverlays(GuiGraphicsExtractor guiGraphics) {
