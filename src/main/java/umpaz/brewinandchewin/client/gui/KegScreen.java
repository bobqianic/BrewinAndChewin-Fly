...
public class KegScreen extends AbstractContainerScreen<KegMenu> implements RecipeUpdateListener {
    ...
    public void extractBackground(GuiGraphicsExtractor gui, int mouseX, int mouseY, float partialTicks) {
        ...
        if (!itemDisplay.isEmpty()) {
            ...
            if (displayedProduct.missing()) {
                int backgroundX = this.leftPos + PRODUCT_AREA.x();
                int backgroundY = this.topPos + PRODUCT_AREA.y();
                gui.fill(
                        backgroundX,
                        backgroundY,
                        backgroundX + PRODUCT_AREA.width(),
                        backgroundY + PRODUCT_AREA.height(),
    ...
    private ItemStack getFluidDisplayItem(AbstractedFluidStack fluidStack) {
        ...
        int pourCount = pouringRecipe.map(kegPouringRecipe -> (int) (Math.min(this.menu.kegTank.getFluidCapacity(), displayedAmount) / kegPouringRecipe.getLoaderAmount())).orElse(1);
